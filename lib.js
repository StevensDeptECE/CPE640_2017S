...
function requestAjax(ajax_url, handler, error, url) {
    ...
    var onsuccess = function (text, didSucceed) {
    ...
    service.getFileFS(ajax_url, false, onsuccess, onsuccess);
...
function getPrefs(ajax_url, error, url) {
    console.log(ajax_url);
    var handler = function (text, didSucceed) {
        if (!didSucceed) {
            if (error)
                error("File not found or does not exist.");
            return;
    ...
    service.getFileFS(ajax_url, false, handler, handler);
